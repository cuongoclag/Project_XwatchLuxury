...
class DongHoNamController extends FrontendController
{
    ...
    public function search(Request $request){
        ...
        $keyword = $request->key;
        $type = DB::table('typeproduct')->orderby('Typeid','desc')->get();
        $search_product = DB::table('product')->where('Status',1)->where('Productname','like','%'.$keyword.'%')->where('Status',1)->get();
        if($request->sort)
        {
            $sort = $request->sort;
            switch($sort)
            {
                case '1':
                    $search_product = DB::table('product')->orderby('Price','desc')->where('Status',1)->get();
                    return  view('frontend.tim_kiem.search')->with('type',$type)->with('search_product',$search_product);;
                break;

                case '2':
                    $search_product = DB::table('product')->orderby('Price','asc')->where('Status',1)->get();
                    return  view('frontend.tim_kiem.search')->with('type',$type)->with('search_product',$search_product);;
                break;

                default:
                    $search_product = DB::table('product')->where('Productname','like','%'.$keyword.'%')->where('Status',1)->get();
                    return  view('frontend.tim_kiem.search')->with('type',$type)->with('search_product',$search_product);;
            }
        }
        return  view('frontend.tim_kiem.search')->with('type',$type)->with('search_product',$search_product);
